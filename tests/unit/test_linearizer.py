...
class TestSaveLoadIntegration:
    ...
    def test_save_and_load_with_complex_model(self, complex_model):
        ...
        with torch.no_grad():
            for param in complex_model.parameters():
                param.fill_(0.0)
            for buf in complex_model.buffers():
                buf.fill_(0.0)

        Linearizer._load_model_state(complex_model, saved_bytes)

        for key in original_state:
            assert torch.allclose(
                complex_model.state_dict()[key], original_state[key]
            ), f"State {key} not restored after round-trip"
